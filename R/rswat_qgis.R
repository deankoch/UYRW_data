...
#' **Mitacs UYRW project** 
#' 
#' **rswat_qgis**: R functions for generating a SWAT+ model via QSWAT+, and reading QSWAT+ files
#' 
#' Construction of a SWAT+ model usually starts in [QGIS3](https://www.qgis.org/en/site/), where
#' georeferenced data are assembled and processed by the 
#' [QSWAT+ plugin](https://swat.tamu.edu/software/plus/) to generate a set of shapefiles and
#' tables that can be interpreted by SWAT+Editor. The latter then creates the SWAT+ text
#' input/output directory and populates it with configuration files needed by the simulation
#' executable.
#' 
#' Since QSWAT+ uses a point-and-click interface, it is difficult to automate and document this
#' model construction process. The functions `qswat_setup` and `qswat_run` help get around this
#' problem, by preparing input files in a standardized, reproducible way (using outputs of the
#' `get_data` workflow) and by running QSWAT+/SWAT+Editor in a
#' [PyQGIS script](https://gitlab.com/rob-yerc/swat) that can be called from R (see also
#' [SWAT+ Automatic Workflow (AW)](https://celray.github.io/docs/swatplus_aw/introduction.html)
#' for a similar, but not R-based. project from Celray James)
#' 
#' The shapefiles produced by QSWAT+ are not strictly necessary for running SWAT+ simulations
#' (only the text input/output directory is needed for that), but they are helpful for
#' visualizing spatial aspects of the model. `qswat_read` and `qswat_plot` are utilities for
#' loading and viewing these files in R.
#' 
#' See [demo_qswat](https://github.com/deankoch/UYRW_data/blob/master/markdown/demo_qswat.md)
...
#' ## dependencies
#' 
#' There should be no R dependencies beyond the libraries listed above, however note that these
#' functions are written specifically to work with output from the `get_data` workflow. We plan
#' to work on generalizing them so that they can be useful outside of the context of the UYRW
#' project.
#' 
#' Users will need to install the long-term release of
#' [QGIS3](https://qgis.org/en/site/forusers/download.html) (64 bit) along with the
#' [SWAT+ installer](https://swat.tamu.edu/software/plus/), which includes QSWAT+, SWAT+Editor,
#' and the SWAT+ model executable.
#'   
#' One of the challenges with this will be to identify paths needed for setting up the PyQGIS
#' environment and running the model. The code below has hard-coded paths that work on my machine
#' with Windows 10, QGIS-LTR, and the latest SWAT+ releases installed with default settings.
#' However it may be a challenge to write a package that reliably finds the SWAT+ executable and
#' OSGEO4W root for any given user. 
#' 
