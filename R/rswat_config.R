...
#' 
#' ## low-level internal workflow to load a SWAT+ config table then write to it
#' 
#' Helper functions for `rswat_open`. The following functions load the SWAT+ config file
#' plaintext, detect name-value pairs, convert them to R dataframes, and build a database
#' of string positioning information about the file so it can be edited.
#' 
#' The functions in steps 0-2 don't return anything - they load file data (`rswat_rfile`),
#' detect field spacing rules (`rswat_rlines`), and coerce to the appropriate class
#' (`rswat_rtext`), copying the results to internal storage in `.rswat$stor`.
#' 
#' Step 3 (`rswat_tfind`) parses this data to detect the first table, adjusting the field
#' spacing rules initialized by `rswat_rlines` as needed, and step 4 (`rswat_tmake`) returns
#' this table as a dataframe. Steps 3-4 and are run repeatedly until the end of the file is
#' reached.
#' 
...
#' step 1: line reader for SWAT+ text files (run once)
...
#' step 2: class detection to interpret SWAT text as R objects (run once)
...
#' step 3: identify first table in a block of SWAT+ config text (run repeatedly)
...
#' step 4: make a table from a parsed SWAT+ file text  (run repeatedly)
...
#' 
#' ## low-level functions for reading inconsistent tables in SWAT+ files
#' 
#' The worflow above relies on spacing patterns in the file which may be violated
#' occasionally. The following functions attempt to repair these problems, but do
#' not modify the files on disk. 

#' repair missing values in incomplete lines of a SWAT+ table (helper for `rswat_rfile`) 
...
#' clean up positional data for a table in `linedf` (helper for `rswat_tmake`) 
...
#' 
#' ## low-level functions for reading exceptional tables in SWAT+ files
#' 
#' The following are for handling exceptional files that don't play nice with the
#' methods above:
#' 
#' In "file.cio" there are no headers, so we need to insert them ourselves before
#' running the `rswat_rfile` workflow. In weather input files, we have a small
#' table of metadata preceeding a (potentially enormous) data table that would take
#' too long to parse using the `rswat_tfind` loop.
...
#' 
#' ## low-level functions for writing SWAT+ files
#' 
