...
#' 
#' This code fetches the data, warps to our reference coordinate system, and prepares input files required
#' by SWAT and SWAT+
#' 
...
#' Filenames (with descriptions) are now stored as a
...
#' This includes a large number of growth parameter sets for agricultural crops (eg. `alfalfa`), and a
#' collection of generic classes for wild plant assemblages (eg. `deciduous_broadleaf_forest`). 
#' 
#' We will use these codes to parametrize HRUs based on BVC information about land use derived from the
#' LANDFIRE/GAP dataset. This section maps
...
#' required by QSWAT+.
#' 
#' The raster will also work with SWAT, but the lookup table will not. At this time, the plant growth
#' parameter set that ships with SWAT is a smaller subset of the one included in SWAT+, and is coded
#' differently (requiring 4-letter uppercase codes). The plant codes assigned by this chunk will
#' therefore not be recognized when running QSWAT. SWAT Users will need to import the missing parameters
#' from the `plants_plt` database in SWAT+ (into 'QSWATRef2012.mdb'), and convert the codes in their
#' lookup table. The script 'make_qswat' does this automatically, later on.
#' 
...
#' First we make a plot of the 10 or so NVC physiognomy classes in the UYRW.
#' In the code (below) we create the raster and legend plot objects separately, then combine them
#' together using (base) R's `grid` package. This works around a bug in which certain legend labels
#' for factor levels in a raster plot are scrambled or omitted.
...
#' Next plot the assigned SWAT+ plant codes in the UYRW (based on NVC biogeography classes). We use
#' the `colorspace` package to create palettes for the large number of different land use types. Note
#' that areas of open water are assigned a no-data value in the SWAT+ land use raster
